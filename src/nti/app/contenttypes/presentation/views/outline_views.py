...
class AssetByOutlineNodeView(AbstractAuthenticatedView):
    ...
    def _allow_assessmentref(self, iface, item):
        ...
        predicate = get_course_assessment_predicate_for_user(
            self.remoteUser, course)
    ...
    def _do_call(self, course, record):
        ...
        if items:
            try:
                last_mod = max(x.lastModified for x in items.values()
            ...
            for item in group or ():
                ...
                for provided in (ICourseOutlineNode, INTILessonOverview):
                    ...
                    if obj is None or not obj.isPublished():
                        unpublished = True
                        break
                # No need to keep checking the group if unpublished and not
                # editor.
